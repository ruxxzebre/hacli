...
const register = (server, options) => {
  ...
  server.decorate('server', 'hapiAuthorization', settings)
...
internals.onPreHandler = async (request, h) => {
  ...
  if (params) {
    ...
    const { hapiAuthorization } = request.server;

    const user = get(request, hapiAuthorization.userPath, false);
    ...
    // If we're using hierarchy
    if (!!hapiAuthorization && hapiAuthorization.hierarchy === true) {
      roleHierarchy = hapiAuthorization.roleHierarchy;
    } else {
      roleHierarchy = false;
    }
